...
export class AppHttpClient {
    ...
    public put<T>(uri: string, params: object = null): Observable<T> {
        return this.httpClient.put<T>(this.prefixUri(uri), params).catch(err => this.errorHandler.handle(err));
    }

    public delete<T>(uri: string, params: object = null): Observable<T> {
        return this.httpClient.delete<T>(this.prefixUri(uri), {body: params} as any).catch(err => this.errorHandler.handle(err));
    }
