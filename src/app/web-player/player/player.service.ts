...
export class Player {
    ...
    public cueTrack(track: Track): Promise<any> {
        ...
        if ( ! track || ! this.playbackStrategy) return new Promise(resolve => resolve());
        this.queue.select(track);
        return this.playbackStrategy.cueTrack(track).then(() => {
    ...
    private setInitialVolume() {
        ...
        this.setVolume(this.storage.get('player.volume', defaultVolume));
    }
    ...
    private async maybeMaximizeOverlay(): Promise<boolean> {
        if (this.playbackStartedViaGesture || ! this.globalState.isMobile) return;
