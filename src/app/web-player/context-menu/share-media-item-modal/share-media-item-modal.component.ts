...
export class ShareMediaItemModalComponent extends BaseModalClass {
    ...
    private getLink(): string {
        if ( ! this.type) return;
        return this.urls.routerLinkToUrl(this.urls[this.type as any](this.mediaItem));
    }
    ...
    private getSocialMediaUrl(type: string) {
        switch(type) {
            ...
            case 'twitter':
                return 'https://twitter.com/intent/tweet?url='+this.getLink()+'&text='+this.mediaItem.name;
            case 'google-plus':
                return 'https://plus.google.com/share?url='+this.getLink();
            case 'pinterest':
                return 'https://pinterest.com/pin/create/button/?url='+this.getLink()+'&media='+this.getImage();
            case 'tumblr':
                let base = 'https://www.tumblr.com/widgets/share/tool?shareSource=legacy&canonicalUrl=&posttype=photo&title=&caption=';
                return base+this.mediaItem.name+'&content='+this.getImage()+'&url='+this.getLink();
            case 'stumbleupon':
                return 'http://www.stumbleupon.com/submit?url='+this.getLink();
            case 'blogger':
                return 'https://www.blogger.com/blog_this.pyra?t&u='+this.getLink()+'&n='+this.mediaItem.name;
        }
    }
